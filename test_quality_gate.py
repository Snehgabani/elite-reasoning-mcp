...
def server_context():
    ...
    process = subprocess.Popen(
        [
            ...
            "-m",
            "uvicorn",
            "core.integration.sync_server:app",
            "--host",
            "127.0.0.1",
            "--port",
            "8002",
...
def test_quality_gate():
    with server_context():
        ...
        try:
            resp = httpx.post(
                "http://127.0.0.1:8002/api/sync/push", json={"anti_patterns": [], "decisions": []}, timeout=30.0
            )
        ...
        spam_ap = {"mistake": "it broke", "root_cause": "bug", "fix": "fixed it", "severity": "low", "tags": "test"}
        ...
        payload = {"anti_patterns": [spam_ap, elite_ap], "decisions": []}
