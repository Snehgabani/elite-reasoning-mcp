...
```
U = success × groundedness × safety × instruction_following
    -----------------------------------------------------
    dollars + λ·latency + μ·human_review_minutes
```
...
Use hierarchical models or cluster-robust intervals when multiple outputs come from the same task, model, or rater. Correct for multiple secondary endpoints. Report all exclusions, missingness, ties, and early stopping. Publish the analysis code and a signed, immutable manifest of prompts, versions, seeds, and outputs after the study.
...
## Highest-priority implementation tickets

1. Rewrite `scripts/double_blind_eval.py` as a real runner with independent generation, secure randomization, true swapped judging, task-level hidden tests, and an analysis manifest.
2. Add `tests/test_evaluation_integrity.py` to fail if treatment labels, hand-coded scores, identical swap passes, or post-hoc task selection appear.
3. Add a safe execution adapter and remove `exec` from the evaluator's threat model.
4. Add claim-level grounding records and make degraded retrieval a hard non-success for research-grade claims.
5. Add a contamination-resistant task ingestion format with temporal metadata and locked test storage.
6. Replace headline metrics in `DOUBLE_BLIND_SCORECARD.md` with pilot results, CIs, limitations, and a ship/hold/reject decision.
7. Add cost-quality Pareto reports and ablations so every extra MCP call must justify its price.

## Implemented in this iteration

The first integrity slice is now implemented in `core/eval/blind_protocol.py` and covered by tests:

- `position_bias_report` reports swap consistency, conflict rate, first-slot preference, and reliable-winner rate without silently converting disagreement into a win.
- `paired_bootstrap_delta_ci` resamples paired treatment-minus-baseline differences and returns a reproducible 95% percentile interval.
- `validate_trial_manifest` fails closed for missing lock/seed/oracle metadata, too-small studies, duplicate case IDs, hand-authored/fixed-score cases, and identical arm outputs.
- The focused MCP verification run executed 11 tests successfully; the complete repository suite executed 288 tests successfully. A remaining non-failing warning is emitted by an upstream Pydantic settings forward-reference check.

This is an integrity foundation, not a completed confirmatory RCT. Real model runs still require a locked holdout, independent generation, objective oracles, and the sample-size/power procedure below.

## Bottom line

The project can become a high-leverage “quality compiler” for cheap models, but the moat will be **measurement integrity and failure containment**, not the number of reasoning modules. First make the evaluation impossible to fool, then optimize the pipeline against fresh real-world tasks. If it cannot show a repeatable quality-per-dollar lift, lower unsupported claims, and safe abstention under blinded testing, adding more cognitive stages will mostly add latency and a more elaborate story.
...
All numeric recommendations in this document are planning guidance or source-reported findings, not results from this repository.
