...
logging.basicConfig(
    filename=os.path.expanduser("~/.gemini/antigravity/scratch/elite-system/proxy_error.log"), level=logging.DEBUG
)
...
def truncate_payload(data):
    if isinstance(data, list):
        if len(data) > MAX_LIST_ITEMS:
            return [truncate_payload(x) for x in data[:MAX_LIST_ITEMS]] + [
                f"... truncated {len(data) - MAX_LIST_ITEMS} items ..."
            ]
    ...
    try:
        for line in iter(proc.stdout.readline, b""):
            try:
                ...
                error_msg = {
                    "jsonrpc": "2.0",
                    "result": f"[PROXY ERROR] Failed to parse payload safely. Truncation aborted to protect context. Error: {str(e)}",
                }
