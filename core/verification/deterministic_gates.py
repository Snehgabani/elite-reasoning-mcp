...
def validate_syntax(code: str, language: str = "python") -> ValidationResult:
    ...
    # --- C-Style / Polyglot Language Bracket & Delimiter Verification (TS, JS, Rust, Go, Java, C/C++) ---
    elif lang in (
        ...
        return _validate_c_like_syntax(code, lang)
...
def _validate_c_like_syntax(code: str, lang: str) -> ValidationResult:
    """Fast, deterministic delimiter and comment-aware syntax validator for C-family languages."""
    brackets = {"(": ")", "{": "}", "[": "]"}
    stack: List[Tuple[str, int, int]] = []  # (expected_closing, line_num, col_num)
    ...
    while idx < n:
        ...
        if char == "\n":
            ...
            in_line_comment = False
            if in_string in ("'", '"'):
                # Newline inside single/double quotes without escape is a syntax error in most languages
                if not escaped and lang not in ("go", "golang"):  # Go backticks allow newlines
                    return ValidationResult(
                        passed=False,
                        score=0.0,
                        issues=[f"{lang.upper()} SyntaxError: Unterminated string literal at line {line_num - 1}"],
                        metadata={"language": lang, "line": line_num - 1},
                    )
            idx += 1
            escaped = False
            continue
        ...
        if in_string is None and char == "/" and idx + 1 < n:
        ...
        if char in brackets:
            stack.append((brackets[char], line_num, col_num))
        elif char in brackets.values():
            if not stack:
                return ValidationResult(
                    passed=False,
                    score=0.0,
                    issues=[f"{lang.upper()} SyntaxError: Unexpected closing bracket '{char}' at line {line_num}, col {col_num}"],
                    metadata={"language": lang, "line": line_num, "col": col_num},
                )
            expected, start_line, start_col = stack.pop()
            if expected != char:
                return ValidationResult(
                    passed=False,
                    score=0.0,
                    issues=[f"{lang.upper()} SyntaxError: Mismatched bracket: expected '{expected}' (opened at line {start_line}, col {start_col}), got '{char}' at line {line_num}, col {col_num}"],
                    metadata={"language": lang, "line": line_num, "col": col_num},
                )
    ...
    if in_block_comment:
        return ValidationResult(
            passed=False,
            score=0.0,
            issues=[f"{lang.upper()} SyntaxError: Unterminated block comment /* ... */"],
    ...
    if in_string:
        return ValidationResult(
            passed=False,
            score=0.0,
            issues=[f"{lang.upper()} SyntaxError: Unterminated string literal ({in_string})"],
            metadata={"language": lang},
        )

    if stack:
        expected, start_line, start_col = stack[-1]
        return ValidationResult(
            passed=False,
            score=0.0,
            issues=[f"{lang.upper()} SyntaxError: Unclosed bracket (expected '{expected}' to close bracket opened at line {start_line}, col {start_col})"],
            metadata={"language": lang, "line": start_line, "col": start_col},
        )
