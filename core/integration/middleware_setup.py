...
def wrap_registered_tools(mcp: FastMCP, chain: MiddlewareChain) -> int:
    ...
    try:
        ...
        logger.info(
            "Middleware chain connected to tools",
            extra={"wrapped": wrapped_count, "exempt": len(EXEMPT_TOOLS), "total": len(tool_manager._tools)},
        )
    except Exception as e:
        logger.error("middleware_setup: failed to wrap tools", extra={"error": str(e)}, exc_info=True)
