def register(mcp, store, orchestrator=None):
    ...
    @mcp.tool()
    def benchmark_track(
        metric: str, value: float = 0, unit: str = "", action: str = "record", context: str = ""
    ) -> str:
        ...
        if action == "trend":
            ...
            status_emoji = {"in_control": "✅", "above_control_limit": "🔴", "below_control_limit": "🔴"}.get(
                t["status"], "⚠️"
            )
