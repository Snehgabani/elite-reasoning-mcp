def register(mcp, store, orchestrator=None):
    ...
    def record_prompt_intent(
        ...
        row_id = store.record_prompt_intent(
            session_id, prompt_text, intent_category, reasoning_type, implicit_expectation, failure_detected
        )
    ...
    @mcp.tool()
    def update_thinking_pattern(pattern_name: str, system_adaptation: str, example_prompt: str = "") -> str:
    ...
    def register_prevention_rule(
        ...
        result = store.register_prevention_rule(
            rule_name, trigger_event, check_query, action_on_match, severity, src_id
        )
        return f"🛡️ {result}"
            ...
            predictions.append(
                {
                    ...
                    "confidence": min(
                        0.9, (severity_counts.get("high", 0) + severity_counts.get("critical", 0)) * 0.15
                    ),
