    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                return AIMessage(
                    content=fallback_content or f"Analysis complete for prompt (LLM proxy unavailable: {exc})"
                )
...
async def cognitive_router_node(state: ReasoningState) -> dict:
    ...
    return {"cognitive_system": res["system"], "retry_count": 0, "execution_status": "PENDING"}
...
async def self_discover_node(state: ReasoningState) -> dict:
    ...
    return {"self_discover_topology": topology}
...
"""
    ...
    response = await _safe_ainvoke(
        POLICY_LLM,
        [SystemMessage(content=system_prompt), HumanMessage(content=f"TASK: {state['task']}")],
...
async def think_on_graph_node(state: ReasoningState) -> dict:
    ...
    return {"tog_facts": res.get("multi_hop_facts", [])}
...
async def research_node(state: ReasoningState) -> dict:
    ...
    return {"research_nodes": [research_entry]}
...
"""
    ...
    try:
        ...
        return {"fact_nodes": [response.content], "assume_nodes": [], "messages": [response]}
...
async def self_rag_node(state: ReasoningState) -> dict:
    ...
    return {"self_rag_reflection": res}
...
async def red_team_node(state: ReasoningState) -> dict:
    ...
    return {"red_team_nodes": [attack_res["antithesis"]], "synthesis_node": synth_res["synthesis"]}
...
async def epistemic_verifier_node(state: ReasoningState) -> dict:
    ...
    return {"assume_nodes": new_assumptions}
...
"""
    ...
    try:
        ...
        conf = "HIGH"
    return {"reflect_confidence": conf, "messages": [response]}
...
async def conclude_node(state: ReasoningState) -> dict:
    ...
    if state.get("cognitive_system") == "SYSTEM_1":
        ...
        return {"conclude_node": ans, "final_answer": ans}
...
"""
    ...
    response = await _safe_ainvoke(
        SOLVER_LLM,
        [SystemMessage(content=system_prompt), HumanMessage(content=full_context)],
    ...
    return {"conclude_node": final, "final_answer": final, "messages": [response]}
