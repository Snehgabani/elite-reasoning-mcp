...
class CounterfactualGenerator:
    ...
    def _random_perturbation(self, step: str) -> str:
        ...
        replacements = ["something", "another thing", "a different value", "an alternative", "a different approach"]
...
class CausalSensitivityChecker:
    ...
    def test_causal_sensitivity(
        self, reasoning_steps: List[str], context: Any, question: str, perturbation_types: List[str] = None
        ...
        for step_idx, step in enumerate(reasoning_steps):
            for perturbation_type in perturbation_types:
                # Perturb this step
                perturbed_step = self.perturbation_generator.perturb(step, perturbation_type)
    ...
    def compute_causal_sensitivity_score(self, test_results: List[CausalTestResult]) -> Dict[str, Any]:
        """
...
class CausalPruner:
    ...
    def prune_reasoning_steps(
        self, reasoning_steps: List[str], context: Any, question: str, sensitivity_threshold: float = 0.5
    ) -> List[str]:
        """
        ...
        test_results = self.causal_checker.test_causal_sensitivity(reasoning_steps, context, question)
...
class CausalReasoningModule:
    ...
    def analyze_and_prune(
        ...
        sensitivity_metrics = self.causal_checker.compute_causal_sensitivity_score(test_results)

        # Prune reasoning steps
        pruned_steps = self.pruner.prune_reasoning_steps(reasoning_steps, context, question, sensitivity_threshold)
