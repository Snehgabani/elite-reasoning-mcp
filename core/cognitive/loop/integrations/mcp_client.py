    ...
    def start(self) -> bool:
        ...
        try:
            self.process = subprocess.Popen(
                self.server_command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
    ...
    def _initialize(self):
        ...
        self._send_request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "loop-by-sg", "version": "6.0.0"},
            },
        )
    ...
    def call_tool(self, tool_name: str, arguments: dict) -> MCPToolResult:
        ...
        try:
            ...
            if "result" in response:
                return MCPToolResult(success=True, output=response["result"], duration_ms=duration, tool_name=tool_name)
            ...
            duration = int((time.time() - start) * 1000)
            return MCPToolResult(success=False, output=None, error=str(e), duration_ms=duration, tool_name=tool_name)
...
class MCPIntegrator:
    ...
    def call_tool(self, full_tool_name: str, arguments: dict) -> MCPToolResult:
        ...
        if "." not in full_tool_name:
            return MCPToolResult(success=False, output=None, error=f"Invalid tool name format: {full_tool_name}")

        server_name, tool_name = full_tool_name.split(".", 1)

        if server_name not in self.clients:
            return MCPToolResult(success=False, output=None, error=f"Server not found: {server_name}")
