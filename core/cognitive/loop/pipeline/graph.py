    ...
    def _run(self, state: PipelineState, store: SingularityStore) -> PipelineState:
        ...
        for line in lines:
            ...
            if any(
                ...
                for kw in ("?", "how", "what", "why", "implement", "fix", "build", "design", "analyze")
            ):
                core_elements.append(line_stripped)
            elif any(
                kw in line_stripped.lower()
                for kw in ("i think", "i feel", "probably", "maybe", "everyone says", "obviously")
            ):
...
class DecomposeNode(PipelineNode):
    ...
    def _run(self, state: PipelineState, store: SingularityStore) -> PipelineState:
        ...
        for sp in subproblems:
            ...
            if patterns:
                sp["anti_patterns"] = [{"mistake": p["mistake"], "fix": p["fix"]} for p in patterns]
                ...
                [
                    {
                        "index": idx,
                        "name": "reproduce",
                        "description": "Reproduce the exact error condition with minimal steps.",
                        "depends_on": [1, 2],
                        "validation": "Error reproduced consistently.",
                    },
                    {
                        "index": idx + 1,
                        "name": "localize",
                        "description": "Binary search or trace to find the exact failure point.",
                        "depends_on": [idx],
                        "validation": "Failure point identified with evidence.",
                    },
                    {
                        "index": idx + 2,
                        "name": "root_cause",
                        "description": "Determine WHY it fails (not just WHERE). Apply five-whys.",
                        "depends_on": [idx + 1],
                        "validation": "Root cause explains all observed symptoms.",
                    },
                    {
                        "index": idx + 3,
                        "name": "fix",
                        "description": "Implement minimal fix that addresses root cause, not symptom.",
                        "depends_on": [idx + 2],
                        "validation": "Fix resolves error. No regressions.",
                    },
                ]
            )
        elif cls.intent == "build":
                ...
                [
                    {
                        "index": idx,
                        "name": "design",
                        "description": "Define interface, data structures, error handling, and invariants.",
                        "depends_on": [1, 2],
                        "validation": "Interface documented. Edge cases enumerated.",
                    },
                    {
                        "index": idx + 1,
                        "name": "implement",
                        "description": "Build incrementally. Each piece testable independently.",
                        "depends_on": [idx],
                        "validation": "Code compiles. Unit tests pass.",
                    },
                    {
                        "index": idx + 2,
                        "name": "integrate",
                        "description": "Connect components. Verify system-level behavior.",
                        "depends_on": [idx + 1],
                ...
                [
                    {
                        "index": idx,
                        "name": "enumerate",
                        "description": "List all viable options with explicit constraints.",
                        "depends_on": [1, 2],
                        "validation": "At least 2 options with pros/cons.",
                    },
                    {
                        "index": idx + 1,
                        "name": "challenge",
                        "description": "Adversarial review of each option from multiple perspectives.",
                        "depends_on": [idx],
                        "validation": "Each option challenged on 3+ axes.",
                    },
                    {
                        "index": idx + 2,
                        "name": "decide",
                        "description": "Select best option with documented rationale and rejected alternatives.",
                        "depends_on": [idx + 1],
                        "validation": "Decision recorded. Rationale self-contained.",
                    },
                ]
            )
        elif cls.intent == "research":
                ...
                [
                    {
                        "index": idx,
                        "name": "gather",
                        "description": "Collect evidence from multiple sources. Verify recency and quality.",
                        "depends_on": [1, 2],
                        "validation": "At least 3 sources. Recency verified.",
                    },
                    {
                        "index": idx + 1,
                        "name": "synthesize",
                        "description": "Map evidence to claims. Assign confidence levels.",
                        "depends_on": [idx],
                        "validation": "Every claim has supporting evidence.",
                    },
                    {
                        "index": idx + 2,
                        "name": "verify",
                        "description": "Cross-check for contradictions. Flag unsupported assertions.",
                        "depends_on": [idx + 1],
                        "validation": "No contradictions. Uncertainty documented.",
                    },
                ]
            )
        else:
            subproblems.append(
                {
                    "index": idx,
                    "name": "execute",
                    "description": "Perform the task with focused execution.",
                    "depends_on": [1, 2],
                    "validation": "Task completed. No unresolved blockers.",
                }
            )
...
class AdversarialVerifyNode(PipelineNode):
    ...
    PERSPECTIVES = [
        {
            "name": "Security",
            "focus": ("auth", "inject", "secret", "permission", "xss", "csrf", "bypass"),
            "lens": "What attack vector does this expose?",
        },
        {
            "name": "Scalability",
            "focus": ("query", "loop", "lock", "memory", "connection", "unbounded"),
            "lens": "Where does this break at 10x scale?",
        },
        {
            "name": "Correctness",
            "focus": ("error", "null", "edge", "boundary", "race", "concurrent"),
            "lens": "What edge case is silently wrong?",
        },
        {
            "name": "Simplicity",
            "focus": ("abstract", "layer", "wrapper", "pattern", "generic", "flexible"),
            "lens": "What simpler solution was overlooked?",
        },
        {
            "name": "Future",
            "focus": ("lock-in", "debt", "coupling", "assumption", "deprecated", "irreversible"),
    ...
    def _run(self, state: PipelineState, store: SingularityStore) -> PipelineState:
        ...
        gates = [
            ...
            {
                "name": "no_unresolved_critiques",
                "status": "pending",
                "check": f"All {len(state.critiques)} critique dimensions pass",
            },
            {
                "name": "adversarial_risks_addressed",
                "status": "pending",
...
class ReasoningPipeline:
    ...
    def run(self, prompt: str, mode: str | None = None) -> PipelineState:
        ...
        try:
            ...
            self.store.record_metric("pipeline_duration_ms", state.pipeline_duration_ms, "ms", {"mode": self.mode})
