...
class CompletePipeline:
    ...
    def run(self, prompt: str, mode: str | None = None) -> PipelineResult:
        ...
        framework, confidence, alternatives = self._select_framework_smart(prompt, classification)
    ...
    def _select_framework_smart(self, prompt: str, classification) -> tuple[str, float, List[str]]:
        ...
        confidence = self._calculate_framework_confidence(selected, intent, complexity, prompt)

        return selected, confidence, alternatives

    def _calculate_framework_confidence(self, framework: str, intent: str, complexity: int, prompt: str) -> float:
...
def verify_local_first():
    ...
    external_apis = ["openai", "anthropic", "requests", "httpx", "aiohttp", "urllib.request", "google", "azure", "aws"]
    ...
    return {"local_first": len(violations) == 0, "violations": violations, "checked_modules": len(sys.modules)}
