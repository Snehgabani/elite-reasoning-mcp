...
class GoalOrientedPipeline:
    ...
    def _execute_step(self, state: ExecutionState):
        ...
        # Store in history
        state.history.append(
            {"type": "execution", "step": current_step.id, "attempt": current_step.attempts, "output": output}
        )
    ...
    def _detect_drift(self, state: ExecutionState) -> bool:
        ...
        recent_executions = [h for h in state.history[-5:] if h.get("type") == "execution"]
    ...
    def _detect_redundancy(self, state: ExecutionState) -> bool:
        ...
        # Get recent executions
        recent_executions = [h for h in state.history[-3:] if h.get("type") == "execution"]
    ...
    def _verify_goal(self, state: ExecutionState) -> bool:
        ...
        for criteria in state.goal.success_criteria:
            ...
            all_outputs = " ".join([h.get("output", "") for h in state.history if h.get("type") == "execution"])
...
"""
    ...
    pipeline = GoalOrientedPipeline(store=store, model_executor=mock_model, verification_methods=verification_methods)

    # Create goal
    goal = Goal(
        id="test_goal",
        description="Build a REST API with authentication",
        success_criteria=["API endpoints created", "Authentication implemented", "Tests passing"],
        max_iterations=5,
    )
