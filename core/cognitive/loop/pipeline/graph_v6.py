...
class ReasoningPipelineV6:
    ...
    def run(self, prompt: str, mode: str = None) -> PipelineStateV6:
        ...
        if effective_mode in ["amplified", "integrated"] and self.llm_client:
            ...
            state.llm_usage = {"duration_ms": llm_response.duration_ms, "usage": llm_response.usage}
    ...
    def _select_techniques(self, classification, route: str) -> list[str]:
        ...
        if route in ["amplified", "integrated"]:
            techniques.extend(
                ["adversarial_verify", "adversarial_self_play", "reasoning_prompt_generator", "quality_score"]
            )
    ...
    def _orchestrate_external_tools(self, prompt: str, state: PipelineStateV6) -> list[dict]:
        ...
        if intent in ["build", "debug", "optimize"]:
            ...
            tool_calls.append(
                {"tool": "code_execution.run", "status": "available", "note": "Code execution MCP would be called here"}
            )
        ...
        if intent == "research":
            ...
            tool_calls.append(
                {"tool": "web_search.query", "status": "available", "note": "Web search MCP would be called here"}
            )
        ...
        if intent in ["build", "debug"]:
            ...
            tool_calls.append(
                {"tool": "filesystem.read", "status": "available", "note": "Filesystem MCP would be called here"}
            )
    ...
    def _run_multi_agent_debate(self, prompt: str, state: PipelineStateV6) -> list[dict]:
        ...
        # Agent A: Proposer
        debate.append({"agent": "proposer", "role": "Generate initial solution", "turn": 1})

        # Agent B: Critic
        debate.append({"agent": "critic", "role": "Identify flaws and weaknesses", "turn": 2})

        # Agent A: Refiner
        debate.append({"agent": "refiner", "role": "Address criticisms and improve solution", "turn": 3})
    ...
    def _real_multi_turn_refinement(self, prompt: str, reasoning_prompt: str, initial_answer: str) -> list[dict]:
        """Run real multi-turn refinement with LLM."""
        history = []

        # Turn 1: Initial answer
        history.append({"turn": 1, "type": "initial", "answer": initial_answer})
        ...
        if self.llm_client:
            ...
            history.append({"turn": 2, "type": "critique", "content": critique_response.content})

            # Generate refinement
            refine_prompt = f"Improve based on critique:\n{critique_response.content}"
            refine_response = self.llm_client.generate(refine_prompt)

            history.append({"turn": 3, "type": "refinement", "answer": refine_response.content})
