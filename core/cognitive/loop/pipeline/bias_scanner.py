...
BIAS_PATTERNS = {
    "anchoring": {
        "signals": ("first approach", "obvious choice", "clearly the best", "naturally",
                     "obviously", "the only way", "everyone knows"),
    ...
    "confirmation_bias": {
        "signals": ("proves that", "confirms", "as expected", "just as I thought",
                     "validates our approach", "supports the decision"),
    ...
    "sunk_cost": {
        "signals": ("already invested", "we've spent", "too far to", "already built",
                     "can't change now", "committed to"),
    ...
    "authority_bias": {
        "signals": ("expert says", "industry standard", "best practice", "everyone uses",
                     "google does it", "netflix uses", "amazon recommends"),
    ...
    "availability_bias": {
        "signals": ("recent outage", "last time", "I just read", "everyone's talking about",
                     "trending", "news about", "went viral"),
        "description": "Overweighting recent/vivid examples",
        "severity": "medium",
        "recommendation": "Check base rates. Is this actually common or just memorable?",
    },
    "survivorship_bias": {
        "signals": ("successful companies", "winners all", "top performers",
                     "the ones that made it", "best-in-class"),
    ...
    "halo_effect": {
        "signals": ("amazing", "perfect", "best ever", "incredible", "game-changing",
    ...
    "bandwagon": {
        "signals": ("everyone is using", "industry is moving", "all the cool kids",
                     "you'd be crazy not to", "no-brainer", "adopted by"),
    ...
    "dunning_kruger": {
        "signals": ("simple fix", "easy solution", "trivial", "just do", "obviously",
                     "no need to", "anyone can"),
...
def detect_sycophancy(answer: str, user_prompt: str) -> float:
    ...
    agreement_signals = ["you're right", "exactly", "absolutely", "great point",
                          "that's correct", "i agree", "you're absolutely right",
                          "spot on", "well said", "perfect analysis"]
    ...
    counterpoint_signals = ["however", "but", "alternatively", "on the other hand",
                             "although", "contradicting", "disagree", "challenge",
                             "risk", "downside", "trade-off", "concern"]
    ...
    hype_signals = ["great idea", "excellent approach", "brilliant", "innovative",
...
def detect_confidence_evidence_gap(answer: str, confidence: float) -> float:
    ...
    evidence_signals = ["because", "evidence", "data shows", "research", "benchmark",
                         "measured", "tested", "verified", "according to", "source",
                         "citation", "documentation", "reference", "proven"]
    evidence_count = sum(1 for s in evidence_signals if s in lower)
    
    # Count uncertainty markers
    uncertainty_signals = ["maybe", "possibly", "uncertain", "unclear", "assumption",
                            "might", "could be", "unclear", "not sure", "estimated"]
