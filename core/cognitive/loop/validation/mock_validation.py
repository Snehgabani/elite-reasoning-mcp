...
class MockLLMResponseGenerator:
    ...
    def _generate_section_content(self, section: str, prompt: str, framework: str) -> str:
        ...
        if "thought" in section_lower or "step" in section_lower:
            return (
                f"This is a detailed reasoning step for: {prompt[:50]}...\n\nLet me work through this systematically."
            )
            ...
            return (
                "Based on my analysis, the answer is: This requires a systematic approach considering multiple factors."
            )
...
class MockValidator:
    ...
    def evaluate_response(self, response: str, benchmark: MockBenchmarkPrompt) -> Dict[str, float]:
        ...
        scores["characteristics"] = (
            characteristic_score / len(benchmark.expected_characteristics) if benchmark.expected_characteristics else 0
        )
        ...
        scores["keywords"] = (
            keyword_score / len(benchmark.ground_truth_keywords) if benchmark.ground_truth_keywords else 0
        )
        ...
        for iteration in range(num_iterations):
            ...
            for benchmark in MOCK_BENCHMARKS:
                ...
                response_with = self.response_generator.generate_with_mcp(
                    benchmark.prompt, state.reasoning_template, state.reasoning_framework
                )
    ...
    def _generate_report(self, results: Dict) -> Dict:
        ...
        improvements = {key: avg_with[key] - avg_without[key] for key in avg_without.keys()}
        ...
        for key in ["overall", "characteristics", "keywords", "structure"]:
            ...
            t_tests[key] = {"t_statistic": t_stat, "p_value": p_value, "significant": p_value < 0.05}
