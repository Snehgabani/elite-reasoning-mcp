...
def run_ab_test(
    ...
    pooled_std = (
        math.sqrt(((n_b - 1) * b_std**2 + (n_e - 1) * e_std**2) / (n_b + n_e - 2)) if (n_b + n_e - 2) > 0 else 0.1
    )
    ...
    if significant and delta > 0 and effect_size in ("medium", "large"):
        recommendation = (
            "STRONG EVIDENCE: Reasoning enhancement measurably improves outcomes. Deploy for this task class."
        )
    elif significant and delta > 0:
        recommendation = (
            "MODERATE EVIDENCE: Enhancement helps but effect is small. Deploy if latency cost is acceptable."
        )
        ...
        recommendation = (
            "WARNING: Enhancement appears to HURT outcomes. Investigate and consider disabling for this task class."
        )
    else:
        recommendation = (
            "NO EVIDENCE: Enhancement has negligible impact. Consider removing for this task class to save latency."
        )
