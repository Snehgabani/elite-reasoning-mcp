...
class DecomposeInput(BaseModel):
    ...
    include_memory: Annotated[bool, Field(default=True, description="Surface relevant past decisions and anti-patterns")]
...
def register(mcp, store: SingularityStore):
    ...
    def reasoning_decompose(
        ...
        if depth == "auto":
            depth = "shallow" if classification.complexity <= 2 else "standard" if classification.complexity <= 4 else "deep"
        ...
        if classification.risk_signals:
            warnings.append(f"Risk signals detected: {', '.join(classification.risk_signals)}. Use reasoning_verify before completion.")
        if anti_patterns:
        ...
        store.log_tool_usage("reasoning_decompose", prompt[:200], json.dumps({"steps": len(steps)}), session_id, duration_ms)
...
def _debug_steps(idx: int, prompt: str) -> list[StepPlan]:
    return [
        StepPlan(idx, "reproduce", "Reproduce the error. Capture exact error message, stack trace, and conditions.",
                 "Error reproduced with exact output.", "Error message matches reported issue.", 2),
        StepPlan(idx+1, "root_cause", "Identify root cause using five-whys or binary search. Document the causal chain.",
                 "Root cause identified with evidence chain.", "Root cause explains all observed symptoms.", 3),
        StepPlan(idx+2, "fix_verify", "Implement minimal fix. Run existing tests. Verify no regressions.",
                 "Fix applied. All tests pass. No new warnings.", "Test suite green. Original error resolved.", 2),
    ]


def _build_steps(idx: int, prompt: str, complexity: int) -> list[StepPlan]:
    steps = [
        StepPlan(idx, "design", "Define interface contract, data structures, and error handling strategy.",
                 "Interface documented. Edge cases enumerated.", "No unhandled error paths.", 2),
        StepPlan(idx+1, "implement", "Implement focused changes. Keep each commit atomic and testable.",
                 "Code written. Unit tests for new behavior.", "Lint clean. Tests pass.", 3),
    ]
    if complexity >= 3:
        steps.append(StepPlan(idx+2, "integration", "Verify integration with existing code. Run full test suite.",
                              "Integration tests pass. No regressions.", "Full suite green.", 2))
    return steps


def _decision_steps(idx: int, prompt: str) -> list[StepPlan]:
    return [
        StepPlan(idx, "enumerate_options", "List all viable options with their constraints and requirements.",
                 "At least 2 options enumerated with pros/cons.", "No option dismissed without reason.", 2),
        StepPlan(idx+1, "adversarial_review", "Challenge each option from security, scalability, and maintenance perspectives.",
                 "Each option challenged on 3+ axes. Weaknesses documented.", "No unaddressed critical weakness.", 3),
        StepPlan(idx+2, "decide_record", "Make decision with documented rationale. Record alternatives rejected and why.",
                 "Decision recorded. Rationale documented. Alternatives preserved.", "Decision rationale is self-contained.", 1),
    ]


def _research_steps(idx: int, prompt: str) -> list[StepPlan]:
    return [
        StepPlan(idx, "gather_evidence", "Collect relevant sources, benchmarks, and prior art. Verify recency.",
                 "At least 3 sources cited. Recency verified.", "No source older than 2 years without justification.", 2),
        StepPlan(idx+1, "synthesize", "Synthesize findings into structured claims with confidence levels.",
                 "Claims mapped to evidence. Confidence assigned.", "Every claim has at least one supporting source.", 3),
        StepPlan(idx+2, "verify_claims", "Cross-check claims for contradictions. Flag unsupported assertions.",
                 "Contradictions resolved. Unsupported claims flagged.", "No contradictions remain.", 2),
    ]


def _deploy_steps(idx: int, prompt: str) -> list[StepPlan]:
    return [
        StepPlan(idx, "pre_deploy_check", "Capture current state. Run smoke tests. Verify rollback plan exists.",
                 "Before-state captured. Smoke tests pass. Rollback documented.", "Rollback procedure tested.", 2),
        StepPlan(idx+1, "deploy", "Execute deployment with monitoring. Watch for error rate spike.",
                 "Deployment completed. Metrics within bounds.", "Error rate < baseline + 10%.", 3),
        StepPlan(idx+2, "post_deploy_verify", "Verify deployment. Run post-deploy smoke tests. Compare metrics.",
                 "Post-deploy tests pass. Metrics nominal.", "All health checks green.", 2),
    ]


def _audit_steps(idx: int, prompt: str) -> list[StepPlan]:
    return [
        StepPlan(idx, "scope_audit", "Define audit scope, boundaries, and success criteria.",
                 "Scope documented. Criteria defined.", "Scope covers all risk areas.", 1),
        StepPlan(idx+1, "execute_audit", "Run audit checks systematically. Document findings with severity.",
                 "All checks executed. Findings categorized.", "No finding without severity rating.", 3),
        StepPlan(idx+2, "report_remediate", "Generate findings report. Prioritize remediation. Create action items.",
                 "Report generated. Actions prioritized.", "All P0/P1 findings have remediation plan.", 2),
    ]


def _general_steps(idx: int, prompt: str, complexity: int) -> list[StepPlan]:
    steps = [
        StepPlan(idx, "execute", "Perform the task with focused execution. Document key decisions.",
                 "Task completed. Key decisions noted.", "No unresolved blockers.", 2),
    ]
    if complexity >= 3:
        steps.append(StepPlan(idx+1, "review", "Review output for completeness, correctness, and edge cases.",
                              "Output reviewed. Edge cases checked.", "No obvious issues remain.", 2))
    return steps
