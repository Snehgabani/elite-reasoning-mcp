    ...
    CAPABILITY_TAXONOMY = {
        "CODING_AND_REPAIR": {
            ...
            "primary_tools": [
                (
                    "cegis_repair",
                    "Automated Counterexample-Guided Inductive Synthesis for syntax/invariant repair",
                    {"file_path": "<target_file>", "failing_code": "<code>", "error_trace": "<trace>"},
                ),
                (
                    "apply_reasoning_diff",
                    "Authenticated HMAC-SHA256 filesystem diff application",
                    {"file_path": "<path>", "diff_content": "<unified_diff>", "attestation_token": "<hmac>"},
                ),
                (
                    "fuzz_symbol",
                    "AST-guided mutation and property testing for functions",
                    {"target_code": "<code>", "property_invariants": ["no_crashes"]},
                ),
            ],
        },
        "DEEP_RESEARCH_GROUNDING": {
            ...
            "primary_tools": [
                (
                    "storm_research",
                    "Stanford STORM multi-perspective dialogue research with web grounding",
                    {"topic": "<research_topic>", "depth": 2},
                ),
                (
                    "evaluate_fact_score",
                    "Atomic FActScore entity grounding and claim evaluation",
                    {"output_text": "<text>", "reference_sources": ["<source_urls>"]},
                ),
                (
                    "live_web_search",
                    "Multi-source parallel search across 4 search providers with deduplication",
        ...
        "DIALECTICAL_DEBATE": {
            ...
            "primary_tools": [
                (
                    "mine_epistemic_divergence",
                    "Computes stance entropy and extracts Pareto invariants across viewpoints",
                    {"perspectives_json": '{"Architect": "...", "SRE": "..."}', "topic": "<topic>"},
                ),
                (
                    "expert_panel",
                    "Convenes decorrelated frontier expert personas to stress-test designs",
                    {"problem": "<problem>", "panel_size": 3},
                ),
        ...
        "TREE_SEARCH_PLANNING": {
            ...
            "primary_tools": [
                (
                    ...
                    "Branching tree-of-thoughts search with PRM step pruning",
                    {"problem": "<problem_statement>", "max_depth": 3},
                ),
                (
                    "compose_reasoning_topology",
                    "Composes dynamic Self-Discover reasoning DAG",
                    {"task": "<task_description>"},
                ),
                (
                    "elite_reason",
                    "Supreme Unified 10-layer cognitive loop pre-hook",
                    {"task": "<task>", "task_type": "hard_problem"},
                ),
            ],
        },
        "VERIFICATION_AND_ATTESTATION": {
            ...
            "primary_tools": [
                (
                    "prm_verify_step",
                    "Process Reward Model invariant step verification",
                    {"step_text": "<intermediate_step_reasoning>"},
                ),
                (
                    "verify_argument",
                    "Formal syllogism, premise validity, and fallacy detection",
                    {"argument": "<argument>"},
                ),
                (
                    "attest_workflow_completion",
                    "Zero-Escape FSM gatekeeper verifying complete proof manifest",
                    {"task_id": "<task_id>", "required_stages_json": "[]"},
                ),
            ],
        },
        "DATABASE_AND_MIGRATIONS": {
            ...
            "primary_tools": [
                (
                    "mcp-server-neon:inspect_database",
                    "Deep database schema introspection and query planning on Neon",
                    {"project_id": "<project_id>"},
                ),
                (
                    "prisma-mcp-server:migrate-dev",
                    "Applies and validates declarative Prisma database migrations",
                    {"name": "<migration_name>"},
                ),
                (
                    "query_sovereign_analytics",
                    "Zero-RAM out-of-core DuckDB SQL engine for data aggregation",
                    {"sql_query": "<sql>"},
                ),
            ],
        },
        "STEALTH_BROWSER_RESEARCH": {
            ...
            "primary_tools": [
                (
                    "playwright-elite:stealth_scrape",
                    "Stealth browser scraping with anti-bot bypass and session harvesting",
                    {"url": "<target_url>"},
                ),
                (
                    "context7:query-docs",
                    "Official documentation lookup for modern libraries and frameworks",
                    {"query": "<query>"},
                ),
                (
                    "chrome-devtools-mcp:lighthouse_audit",
                    "Full Core Web Vitals (LCP, INP, CLS) performance audit",
                    {"url": "<url>"},
                ),
            ],
        },
        "ENTERPRISE_ISSUE_SYNC": {
            ...
            "primary_tools": [
                (
                    "atlassian-mcp-server:getJiraIssue",
                    "Fetches sprint acceptance criteria and blocker status from Jira",
                    {"issueIdOrKey": "<key>"},
                ),
                (
                    "mcp-server-github:create_issue",
                    "Creates tracked GitHub issue with reproduction steps",
                    {"owner": "<owner>", "repo": "<repo>", "title": "<title>"},
                ),
                (
                    "atlassian-mcp-server:createConfluencePage",
                    "Publishes verified architectural decision record (ADR) to Confluence",
