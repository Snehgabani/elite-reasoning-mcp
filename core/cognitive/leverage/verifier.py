...
async def verify_code_candidate(
    task: str, candidate_code: str, test_command: Optional[str] = None, sandbox_mode: str = "local"
) -> VerificationResult:
    if not candidate_code.strip():
        return VerificationResult(passed=False, score=0.0, output="Empty candidate code", failure_type="empty_code")

    # 1. Syntax check
    try:
        ast.parse(candidate_code)
    except SyntaxError as e:
        return VerificationResult(passed=False, score=0.0, output=f"SyntaxError: {str(e)}", failure_type="syntax_error")
    ...
    if test_command:
        try:
            ...
            return VerificationResult(passed=False, score=0.0, output=str(e), failure_type="execution_error")
    ...
    try:
        ...
        return VerificationResult(passed=False, score=0.0, output=str(e), failure_type="timeout_error")
    finally:
        try:
            os.unlink(tmp_path)
...
async def verify_non_code_candidate(task: str, candidate_answer: str, rubric: List[str]) -> VerificationResult:
    if not candidate_answer.strip():
        return VerificationResult(passed=False, score=0.0, output="Empty response", failure_type="empty_response")
