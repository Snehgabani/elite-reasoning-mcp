...
class RuleLifecycle:
    ...
    def tick(self) -> dict:
        ...
        for rule in rules:
            ...
            if new_state and new_state != rule.lifecycle_state:
                ...
                logger.info(f"Rule '{rule.rule_name}' transitioned: {rule.lifecycle_state} → {new_state} ({reason})")
    ...
    def record_rule_outcome(self, rule_id: int, was_useful: bool):
        ...
        try:
            ...
            c.execute(f"UPDATE prevention_rules SET {col} = COALESCE({col}, 0) + 1 WHERE id = ?", (rule_id,))
        finally:
        ...
        try:
            ...
            return [
                RuleState(
                    ...
                    false_positive_count=r[5],
                    true_positive_count=r[6],
