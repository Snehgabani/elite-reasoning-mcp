...
| `ELITE_USER_ID` | `local-user` | Optional opaque pseudonym for legacy sync; read at each startup and never defaults to the OS account name. |
| `ELITE_DISPLAY_NAME` | `Local User` | Optional display label paired with an explicit pseudonym; read at each startup. |
