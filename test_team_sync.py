...
def test_team_sync():
    ...
    server_process = subprocess.Popen(
        ...
        env={
            **os.environ,
            "ELITE_CENTRAL_DIR": "test_brain_central",
            "SYNC_PORT": "8099",
    ...
    # Push from client 1
    local_payload_1 = {"anti_patterns": store1.get_all_anti_patterns(), "decisions": store1.get_all_decisions()}
    headers = {"X-Elite-Sync-Key": "test-sync-key"}
    push_resp = httpx.post("http://localhost:8099/api/sync/push", headers=headers, json=local_payload_1, timeout=30.0)
    ...
    pull_resp = httpx.get("http://localhost:8099/api/sync/pull", headers=headers, timeout=30.0)
